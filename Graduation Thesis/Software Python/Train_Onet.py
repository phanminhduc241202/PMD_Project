...
# Import the provided ListDataset and ONet classes
...
class ONet(nn.Module):
    def __init__(self, is_train=False):
        ...
        self.conv6_1 = nn.Linear(256, 2)  # Classification
        self.conv6_2 = nn.Linear(256, 4)  # Bounding box regression
        self.conv6_3 = nn.Linear(256, 10) # Landmark regression

    def forward(self, x):
        x = self.features(x)
        a = self.conv6_1(x)  # Classification
        b = self.conv6_2(x)  # Bounding box
        c = self.conv6_3(x)  # Landmarks
    ...
    for phase in ['train', 'val']:
        ...
        for i_batch, sample_batched in enumerate(dataloaders[phase]):
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                # Landmark loss (for label = -2, if applicable, though typically not used for ONet)
                ...
                running_loss_cls += cls_loss.item() * batch_size_actual if len(valid_gt_label) > 0 else 0.0
...
torch.save(model.state_dict(), "C:/Minh_Duc/MD_Personal/LVTN/Training_MTCNN/weights/Onet_weight.pth")
...
# Set environment variable to avoid duplicate library error
...
# Load models (assuming MTCNN_nets.py defines PNet, RNet, ONet)
...
rnet.load_state_dict(torch.load('C:/Minh_Duc/MD_Personal/LVTN/Training_MTCNN/weights/Rnet_weight_v2.pth', map_location=device))
onet.load_state_dict(torch.load('C:/Minh_Duc/MD_Personal/LVTN/Training_MTCNN/weights/Onet_weight_v2.pth', map_location=device))
...
# Parameters
min_face_size = 20.0
thresholds = [0.6, 0.5, 0.5]  # Adjusted O-Net threshold
nms_thresholds = [0.7, 0.8, 0.8]
...
def nms(boxes, overlap_threshold=0.5, mode='union'):
    ...
    ids = np.argsort(score)  # Sort by score in ascending order
    while len(ids) > 0:
        last = len(ids) - 1
        i = ids[last]
        pick.append(i)
...
# Correct bounding boxes to fit within image dimensions
def correct_bboxes(bboxes, width, height):
    ...
    # Adjust if box exceeds right edge
    ind = np.where(ex > width - 1.0)[0]
    edx[ind] = w[ind] + width - 2.0 - ex[ind]
    ex[ind] = width - 1.0
    # Adjust if box exceeds bottom edge
    ind = np.where(ey > height - 1.0)[0]
    edy[ind] = h[ind] + height - 2.0 - ey[ind]
    ey[ind] = height - 1.0
    # Adjust if box is too far left
    ind = np.where(x < 0.0)[0]
    dx[ind] = 0.0 - x[ind]
    x[ind] = 0.0
    # Adjust if box is too high
...
with torch.no_grad():
    for scale in scales:
        ...
        probs = prob.cpu().data.numpy()[0, 1, :, :]  # Face probability
        offsets = offset.cpu().data.numpy()  # Bounding box regression offsets
        ...
        if inds[0].size == 0:
            ...
            offsets = np.array([tx1, ty1, tx2, ty2]).T  # Shape: [num_boxes, 4]
            score = probs[inds[0], inds[1]]
            bounding_box = np.vstack([
                np.round((stride * inds[1] + 1.0) / scale),  # x1
                np.round((stride * inds[0] + 1.0) / scale),  # y1
                np.round((stride * inds[1] + 1.0 + cell_size) / scale),  # x2
                np.round((stride * inds[0] + 1.0 + cell_size) / scale),  # y2
                score
            ]).T
            bounding_box = np.hstack([bounding_box, offsets])  # [x1, y1, x2, y2, score, offsets]
            boxes = bounding_box
...
if num_boxes > 0:
    ...
    offsets = offset.cpu().data.numpy()  # [num_boxes, 4]
    probs = prob.cpu().data.numpy()  # [num_boxes, 2]
    ...
    if len(keep) > 0:
        ...
        bboxes = np.array([])
        print("R-Net: No boxes passed the threshold")
else:
    bboxes = np.array([])
...
rnet_img = image.copy()
for i in range(bboxes.shape[0] if bboxes.size > 0 else 0):
    bbox = bboxes[i, :4]
    cv2.rectangle(rnet_img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (0, 255, 0), 1)

# Stage 3: O-Net (Including Landmarks)
size = 48
num_boxes = len(bboxes) if bboxes.size > 0 else 0
...
if num_boxes > 0:
    ...
    img_boxes = torch.FloatTensor(img_boxes).to(device)
    landmarks, offset, prob = onet(img_boxes)  # O-Net outputs landmarks, offsets, and probabilities
    landmarks = landmarks.cpu().data.numpy()  # [num_boxes, 10], normalized coordinates
    offsets = offset.cpu().data.numpy()  # [num_boxes, 4]
    probs = prob.cpu().data.numpy()  # [num_boxes, 2]
    ...
    if len(keep) > 0:
        bboxes[:, 4] = probs[keep, 1].reshape((-1,))
        offsets = offsets[keep]
        landmarks = landmarks[keep]  # [num_boxes, 10]: [x1, x2, ..., x5, y1, y2, ..., y5]
        ...
        landmarks[:, 0:5] = np.expand_dims(xmin, 1) + np.expand_dims(width, 1) * landmarks[:, 0:5]  # x coordinates
        landmarks[:, 5:10] = np.expand_dims(ymin, 1) + np.expand_dims(height, 1) * landmarks[:, 5:10]  # y coordinates
        print(f"O-Net: {bboxes.shape[0]} boxes after processing")
    else:
        bboxes = np.array([])
        landmarks = np.array([])
        print("O-Net: No boxes passed the threshold")
else:
    bboxes = np.array([])
    landmarks = np.array([])
    print("O-Net: No boxes to process")

# Visualize O-Net results with landmarks
onet_img = image.copy()
for i in range(bboxes.shape[0] if bboxes.size > 0 else 0):
    bbox = bboxes[i, :4]
    cv2.rectangle(onet_img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (255, 0, 0), 2)
    # Draw landmarks: [left_eye, right_eye, nose, left_mouth, right_mouth]
    if landmarks.size > 0:
        for j in range(5):  # 5 landmarks
            x, y = int(landmarks[i, j]), int(landmarks[i, j + 5])
            cv2.circle(onet_img, (x, y), 2, (0, 255, 0), -1)
...
# O-Net with landmarks
plt.subplot(1, 3, 3)
plt.imshow(onet_img[:, :, ::-1])
plt.title('O-Net: Final Boxes with Landmarks')
plt.axis('off')

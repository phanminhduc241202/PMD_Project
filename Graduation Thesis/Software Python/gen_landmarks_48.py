...
image_id = 0
target_images = 3000  # Desired number of generated images

# Output directories
landmark_imgs_save_dir = os.path.join(data_dir, "C:/Minh_Duc/MD_Personal/LVTN/Training_MTCNN/output/landmark").replace("\\", "/")
...
for annotation in annotations:
    ...
    for i in range(15):
        ...
        if iou > 0.65:
            ...
            if l_idx >= target_images:
                print(f"Reached target of {target_images} landmark images. Stopping.")
                break

    # Break the outer loop if the target is reached
    if l_idx >= target_images:
        break

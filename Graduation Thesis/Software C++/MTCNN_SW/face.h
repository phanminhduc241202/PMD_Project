...
struct Face {
    BBox bbox;
    float score; // do tin cay cua phat hien khuon mat
    float regression[NUM_REGRESSIONS]; // bo he so dieu chinh hop gioi han
    float ptsCoords[2 * NUM_PTS]; // danh sach 5 diem dac trung tren khuon mat (mat, mui, mieng)

    // ham applyRegression cap nhat bbox bang he so regression
    // x1′=x1+regression[1]×width
    // y1′=y1+regression[0]×height
    // x2′=x2+regression[3]×width
    // y2′=y2+regression[2]×height
    ...
    // neu addOne = true, kich thuoc hop gioi han duoc tang them 1

    // ham chuyen all bbox thanh Squares
    ...
    // ham runNMS() - Loc Non-Maximum Suppression
    // loai bo cac bbox trung nhau dua tren dien tich giao nhau
    static std::vector<Face> runNMS(std::vector<Face>& faces, float threshold,
        ...
        // sap xep cac face theo do tin cay score giam dan
        // duyet tung khuon mat, so sanh dien tich giao nhau voi threshold
        // su dung cong thuc tinh chong lap IOU
        // neu IOU < threshold thi face dc giu lai

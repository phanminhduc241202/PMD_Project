...
int main(int argc, char **argv){
	...
	cv::Mat resizedImage;
	cv::Mat RGBImage;
	float input[INPUT_SIZE * INPUT_SIZE * CONV1_IN_CHANNEL];
	float out1[FC2_1_DENSE_SIZE] = {0};
	float out2[FC2_2_DENSE_SIZE] = {0};
	

	cv::resize(inputImage, resizedImage, cv::Size(24,24), 0, 0, cv::INTER_LINEAR);
    ...
    rnet_accel(input, conv_mp_2_weights, conv_3_weights, dense_1_weights, out1, out2);
